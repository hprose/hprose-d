...
class Reader : RawReader {
	private {
		...
		T readVariant(T)(char tag) if (is(Unqual!T == Variant)) {
			switch(tag) {
				case '0': .. case '9': return Variant(cast(int)(tag - '0'));
				case TagInteger: return Variant(readIntegerWithoutTag!int());
				case TagLong: return Variant(readBigIntWithoutTag!BigInt());
				case TagDouble: return Variant(readDoubleWithoutTag!double());
				case TagNaN: return Variant(double.nan);
				case TagInfinity: return Variant(readInfinityWithoutTag!double());
				case TagNull: return Variant(null);
				case TagEmpty: return Variant("");
				case TagTrue: return Variant(true);
				case TagFalse: return Variant(false);
				case TagUTF8Char: return Variant(readUTF8CharWithoutTag!string());
				case TagString: return Variant(readStringWithoutTag!string());
				case TagDate: return Variant(readDateWithoutTag!SysTime());
				case TagTime: return Variant(readTimeWithoutTag!TimeOfDay());
				case TagGuid: return Variant(readUUIDWithoutTag!UUID());
				case TagList: return Variant(readArrayWithoutTag!(Variant[])());
				case TagMap: return Variant(readAssociativeArrayWithoutTag!(Variant[Variant])());
				case TagRef: return readRef!Variant();
				default: throw unexpectedTag(tag);
			}
		}
		void readClass() {
	    ...
	    T unserialize(T)(char tag) if (isSerializable!T) {
			...
			else static if (is(Unqual!T == Variant)) {
